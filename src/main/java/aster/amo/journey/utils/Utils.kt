...
object Utils {
    ...
    fun printDebug(message: String, bypassCheck: Boolean = false) {
        if (bypassCheck || ConfigManager.CONFIG.debug)
            Journey.LOGGER.info("[${Journey.MOD_NAME}] DEBUG: $message")
    }

    fun printError(message: String) {
        Journey.LOGGER.error("[${Journey.MOD_NAME}] ERROR: $message")
    }

    fun printInfo(message: String) {
        Journey.LOGGER.info("[${Journey.MOD_NAME}] $message")
    }
    ...
    data class CodecSerializer<T>(val codec: Codec<T>) : JsonSerializer<T>, JsonDeserializer<T> {
        ...
        override fun deserialize(json: JsonElement?, typeOfT: Type?, context: JsonDeserializationContext?): T? {
            return try {
                codec.decode(JsonOps.INSTANCE, json).getOrThrow() { return@getOrThrow null }.first
            } catch (e: Throwable) {
        ...
        override fun serialize(src: T?, typeOfSrc: Type?, context: JsonSerializationContext?): JsonElement {
            return try {
                if (src != null)
                    codec.encodeStart(JsonOps.INSTANCE, src).getOrThrow() { return@getOrThrow null }
                else
                    JsonNull.INSTANCE
            } catch (e: Throwable) {
...
fun <K, V> LinkedHashMap<K, V>.moveToTop(key: K) {
    if (this.containsKey(key)) {
        val value = this.remove(key)!!
        val newMap = LinkedHashMap<K, V>()
        newMap[key] = value
        this.forEach { newMap[it.key] = it.value }
        this.clear()
        this.putAll(newMap)
    }
}
